...
> There is a maximum level of greenness/growth that the environment is able to sustain. After a fire, there is very little competition and plenty of seeds opening as many fynbos species only germinate after fire. There is plenty of new growth and the NDVI increases rapidly. But as the number and size of plants increases, the enviornment will approach carrying capacity and competition will increase, slowing growth. This is seen in the saturating NDVI.
<br>
...
>In the simpler model, $\alpha$ is 0.25107, and in the full model, it's lower, at 0.207522. The simple model reports $\gamma$ as 0.32371 while the full model reports it as 0.364746. The simple model reports $\lambda$ as 1.17687 and the full model reports it as  0.989154.
Both $\lambda$ values have quite high standard error values, at 0.21396 and 0.126064 respectively, Their estimates are larger by several times than the other common parameters, so larger errors are expected, but these are large errors regardless.
<br>
...
>The full model is better as it has a smaller residula sum of squares, at 0.45280 where the simple model has a much larger RSS of 0.71976.

Q4: How many degrees of freedom are there in this ANOVA and why (i.e. what are they)?

>There are two degrees of freedom between these two models, because the simple model does not include $\phi$ and $A$ as parameters.
<br>
...
>The full model is vastly preferred, with the difference of AIC being 266957.8. 
<br>
...
>The NLS models are not fitted by maximum likelihood and they therefore do not meet the requirements for AIC comparison. Because log-likelihood has not been maximized, AIC would be working off two different, non-comparable things and would not give useful results.
<br>